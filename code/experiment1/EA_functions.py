## May 2024
## Functions for the EA where we would only have the distribution of the
## data and some regression coefficients
...
## Input: population scores, child scores, the population and the children (list of DFs)
## Output: population and their score

### where we are minimising the score and there's only one score. This gets the lowest scores
def next_gen_min(pop_scores, child_scores, population, children):
    popn_size = len(population)                                    # get the population size
    population = population + children                             # combine parent & child populations
    scores_com = pop_scores + child_scores                         # combine parent & child scores
    
    if len(set(scores_com)) < 2: 
        indx = np.random.permutation(popn_size*2)[:popn_size]      # if scores are all the same, choose randomly
    else:
        indx = list((np.array(scores_com)).argsort()[:popn_size])  # get index of lowest scores
        
    population = list(population[i] for i in indx)                 # keep the top datasets
    scores_com = list(scores_com[i] for i in indx)                 # keep the corresponding similarity scores
...
##         output1 = the original output, which is regression coefficients in a pandas table
##         catvars = list of categorical variables (for regression)
##         numvars = list of numerical variables (for regression)
##         reflevels = list of reference levels for the regression
##         le = the initiliazed label encoder for one-hot encoding
##         scaler = the initialized scaler for scaling the numerical variables
##         errortype = default mean squared error ('mse'), could also use mean absolute error ('mae')
...
##         verbose = will save the scores in a csv file
...
def setup_initial_pop(source_distribution, pop_size, initial_pop_dn,rand_seed,filename,
                      output1,catvars, numvars, target, reflevels, le, scaler, errortype='mse', 
    ...
    scores = get_all_scores(initial_pop,output1,catvars, numvars, target, reflevels, le, scaler,
                            errortype,jlib_num_cores=joblib_num_cores,jlib_max_nbytes=joblib_max_nbytes)

    if verbose==True:
        print(f'Scores calculated') 

    write_scores(scores, filename)
...
def get_all_scores(population,output1,catvars, numvars, target, reflevels, le, scaler,
                   errortype='mse', jlib_num_cores=-2,jlib_max_nbytes='10M'):
    scores = urfuncs.get_reg_error_paral(output1,population,catvars, numvars, target, reflevels, le, scaler,
                                         errortype,jlib_num_cores,jlib_max_nbytes)
    return scores
...
##         filename = csv file to store the scores in
##         filename_ch = csv file to store the children scores in
##         output1 = the original output coefficients for the regression
##         catvars = list of the categorical columns in the synthetic dataframe
##         numvars = list of the numerical columns in the synthetic dataframe
##         target = the target variable for the regression
##         reflevels = the reference levels for the regression
##         le = the initiliazed label encoder for one-hot encoding
##         scaler = the initialized scaler for scaling the numerical variables
##         errortype = mean squared error ('mse') by default
...
##         mutation_vars = list of variables to mutate (if None, all are mutated)
...
def GA_parallel(pop, scores_pop, pop_size, source_distribution,filename=None,filename_ch=None,
                output1=None,catvars=None, numvars=None, target=None, reflevels=None, 
                le=None, scaler=None, errortype='mse',joblib_num_cores=-2, joblib_max_nbytes='10M',
                cross_rate=0, cross_prob=None, mut_rate=None, mut_dist='univariate',mutation_vars=None,
    ...
    for _ in range(pop_size//2):
        ...
        p1_id, p2_id = selection_min(scores_pop), selection_min(scores_pop)
        while p1_id == p2_id: # if p2_id is the same as p1_id, choose again
            p2_id = selection_min(scores_pop)
        # do crossover
    ...
    children = Parallel(n_jobs=joblib_num_cores,max_nbytes=joblib_max_nbytes)(delayed(mutation_select)(_,source_distribution,mut_rate,mut_dist,weights=False,variables_select=mutation_vars) for _ in children)

    ##### Get scores for children
    scores_ch = get_all_scores(children,output1,catvars, numvars, target, reflevels, 
                               le, scaler, errortype,joblib_num_cores,joblib_max_nbytes)
    
    write_scores(scores_ch, filename_ch)
    ...
    if next_gen == 'children':
        ...
        pop, scores = next_gen_min(scores_pop, scores_ch, pop, children)
